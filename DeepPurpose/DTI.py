...
class DBTA:
	...
	def train(self, train, val = None, test = None, verbose = True):
		...
		for epo in range(train_epoch):
			...
			self.early_stopping(
				{'val_AUC': auc, 'val_AUPRC': auprc, 'val_f1': f1, 'val_loss': loss} if self.binary else {'val_MSE': mse, 'val_pearson_correlation': r2, 'val_concordance_index': CI, 'val_loss': loss_val.item()},
				epo
			)
